...
//	}
	...
	//先序遍历
	private void preOrderTraverse(Node root) {//node7
		if(root != null){
			//输出根的值
			System.out.print(root.value+"  ");//7
			//遍历左子树
			this.preOrderTraverse(root.leftChild);//null
			//遍历右子树
			this.preOrderTraverse(root.rightChild);//null
		}		
	}
	@Override
	...
	public void postOrderByStack() {
		...
		// 创建栈
		Deque<Node> stack = new LinkedList<Node>();
		Deque<Node> stackOut = new LinkedList<Node>();//另外创建一个栈用来存储数据
		Node current = root;
		while (current != null || !stack.isEmpty()) {
			while (current != null) {
		...
		while(stackOut.size()>0){
			System.out.print(stackOut.pop().value+" ");
		}
		System.out.println();	
